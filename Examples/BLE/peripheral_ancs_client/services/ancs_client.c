                      ...
                      bt_ancs_ns_notif_cb func)
{
    ...
    if (ancs_c->state & (1 << ANCS_NS_NOTIF_ENABLED) != 0) {
        return -EALREADY;
    }

    ancs_c->state |= (1 << ANCS_NS_NOTIF_ENABLED);
                  ...
                  bt_ancs_ds_notif_cb func)
{
    ...
    if (ancs_c->state & (1 << ANCS_DS_NOTIF_ENABLED) != 0) {
        return -EALREADY;
    }

    ancs_c->state |= 1 << ANCS_DS_NOTIF_ENABLED;
...
int bt_ancs_notification_action(struct bt_ancs_client *ancs_c, uint32_t uuid,
                ...
                bt_ancs_write_cb func)
{
    if (ancs_c->state & (1 << ANCS_CP_WRITE_PENDING) != 0) {
        return -EBUSY;
    }
